...
export function runDeterministicSimulation(seed=1,hard=false,rivalCount=15){
  let value=seed>>>0;const random=()=>{value=(value*1664525+1013904223)>>>0;return value/4294967296;};
  const count=clamp(Math.round(rivalCount),1,15),companies=Array.from({length:count+1},(_,id)=>({id,budget:id===count?10:hard?20:10,models:[],users:0,subscribers:0,requests:0,points:0,streak:0,insolvent:false}));
