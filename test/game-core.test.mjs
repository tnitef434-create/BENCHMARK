...
import {AD_CAMPAIGNS,INVESTORS,classCap,developmentTime,emptyLedger,forecastLabel,inferenceCost,investmentDecision,runDeterministicSimulation,spendableBudget,subscriptionConversion,weakReleaseFactor} from "../public/game-core.js";
...
test("100 seeded Standard and Hard simulations always finish cleanly",()=>{for(const hard of [false,true])for(let seed=1;seed<=100;seed++){const result=runDeterministicSimulation(seed,hard);assert.equal(result.rounds,16);assert.equal(result.companies.length,16);result.companies.forEach(c=>{assert.ok(Number.isFinite(c.budget));if(!hard)assert.ok(c.budget>=.1);assert.ok(c.requests<=3);if(c.models.length){assert.ok(c.users>=c.models.length*25);assert.ok(c.models.every(m=>m.users>=25));}});}});
test("app contains v5 migration, observer endings and no payment code",()=>{const app=readFileSync(new URL("../public/app.js",import.meta.url),"utf8"),html=readFileSync(new URL("../public/index.html",import.meta.url),"utf8");assert.match(app,/version:\s*5/);assert.match(app,/function showBankruptcy/);assert.match(app,/function simulateToEnd/);assert.match(app,/function awardTopThreeStreaks/);assert.match(html,/id="setupScreen" class="setup-screen is-hidden"/);assert.match(app,/else \$\("#setupScreen"\)\.classList\.remove\("is-hidden"\)/);assert.doesNotMatch(app+html,/Paddle|hard-mode\/verify|Unlock Hard Mode/);});
